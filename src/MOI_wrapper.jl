...
MOI.get(::Optimizer, ::MOI.SolverName) = "PipsNlp"
...
# In setting up the data for PIPS, we order the constraints as follows:
...
function MOI.get(model::Optimizer, ::MOI.TerminationStatus)
    ...
    elseif status == :MAX_ITS_EXCEEDED
        return MOI.ITERATION_LIMIT
    elseif status == :NEED_FEASIBILITY_RESTORATION || :NOT_FINISHED || :UNKNOWN
        return MOI.NUMERICAL_ERROR
    else
        error("Unrecognized PIPS status $status")
    end
end
